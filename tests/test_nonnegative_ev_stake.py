...
def test_nonnegative_ev_stake_rule():
    assert stake_for_ev(-0.06).eligible is False
    assert stake_for_ev(-0.001).eligible is False
    assert stake_for_ev(-0.001).stake == 0.0
    assert stake_for_ev(0.0).eligible is True
    assert stake_for_ev(0.0).stake == 10.0
    assert stake_for_ev(0.049999).stake == 10.0
    assert stake_for_ev(0.05).eligible is True
    assert stake_for_ev(0.05).stake == 15.0
    assert stake_for_ev(0.30).stake == 15.0
    assert stake_for_ev(0.30).tier == "HIGH_EV"


def test_default_strategy_parameters():
    assert SETTINGS.min_trade_ev == 0.0
    assert SETTINGS.stake_mid == 10.0
    assert SETTINGS.stake_high == 15.0
    assert SETTINGS.stake_mid_ev == 0.0
    assert SETTINGS.stake_high_ev == 0.05
