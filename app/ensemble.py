...
def typical_move(rounds: list[dict[str, Any]], snapshots: list[dict[str, Any]]) -> float:
    prior = [abs(float(row.get("live_move_points") or 0)) for row in snapshots if row.get("live_move_points")]
    if len(prior) >= 3:
...
def price_component(
    ...
    ordered = sorted(same_epoch_snapshots, key=lambda row: int(row.get("seconds_to_lock", 0)), reverse=True)
    momentum = 0.0
    acceleration = 0.0
    if ordered:
    ...
    if len(ordered) >= 3:
        ...
        first_leg = (middle_price - float(ordered[0].get("chainlink_price") or middle_price)) / scale
        second_leg = (snapshot.chainlink_price - middle_price) / scale
        acceleration = second_leg - first_leg
    score = 0.65 * _tanh(move_norm / 1.4) + 0.25 * _tanh(momentum / 1.2) + 0.10 * _tanh(acceleration)
    probability = clip(0.5 + 0.15 * score, 0.35, 0.65)
    freshness = clip(1.0 - max(0, snapshot.oracle_age_seconds - 20) / 80.0, 0.25, 1.0)
    return ComponentSignal(
        "price", probability, freshness, True, "chainlink_live_move_momentum",
        {"typical_move": scale, "move_norm": move_norm, "momentum_norm": momentum, "acceleration": acceleration},
    )


def binance_component(data: dict[str, Any]) -> ComponentSignal:
    if not data.get("available"):
        return ComponentSignal("binance", 0.5, 0.0, False, str(data.get("reason", "unavailable")), data)
    ...
    score = (
        0.28 * _tanh(r15 / 8.0) + 0.22 * _tanh(r30 / 12.0) + 0.15 * _tanh(r60 / 18.0)
        + 0.15 * _tanh(taker * 2.5) + 0.10 * _tanh(book * 2.0)
        + 0.06 * _tanh(ema1 / 8.0) + 0.04 * _tanh(ema5 / 12.0)
    )
    probability = clip(0.5 + 0.12 * score, 0.38, 0.62)
    return ComponentSignal("binance", probability, 1.0, True, "binance_microstructure", data)


def crowd_component(snapshot: FusionSnapshot, same_epoch_snapshots: list[dict[str, Any]]) -> ComponentSignal:
    imbalance = (snapshot.betting_bull_share_pct - snapshot.betting_bear_share_pct) / 100.0
    share_change = 0.0
    pool_growth = 0.0
    if same_epoch_snapshots:
        oldest = max(same_epoch_snapshots, key=lambda row: int(row.get("seconds_to_lock", 0)))
        share_change = (snapshot.betting_bull_share_pct - float(oldest.get("betting_bull_share_pct") or 50.0)) / 100.0
    ...
    pool_reliability = clip(snapshot.betting_total_bnb / max(settings.min_pool_bnb, 0.01), 0.15, 1.0)
    return ComponentSignal(
        "crowd", probability, pool_reliability, True, mode,
        {"imbalance": imbalance, "bull_share_change": share_change, "pool_growth": pool_growth, "price_sign": price_sign},
    )


def m9_component(rounds: list[dict[str, Any]]) -> ComponentSignal:
    if len(rounds) < 9:
        return ComponentSignal("m9", 0.5, 0.0, False, "insufficient_history", {"rounds": len(rounds)})
    ...
    return ComponentSignal(
        "m9", clip(probability, 0.38, 0.62), reliability, True,
        "bayesian_exact_state_with_decay",
        {"state": list(suffix), "coefficient_zone": zone, "matches": matches, "up_weight": up_weight, "down_weight": down_weight},
    )
...
def pattern_component(rounds: list[dict[str, Any]]) -> ComponentSignal:
    ...
    for length in range(3, max_length + 1):
        ...
        if (p_first - 0.5) * (p_second - 0.5) <= 0:
            continue
        up = 5.0 + sum(math.exp(-(len(winners)-idx)/500.0) for idx, outcome in outcomes if outcome == "UP")
        down = 5.0 + sum(math.exp(-(len(winners)-idx)/500.0) for idx, outcome in outcomes if outcome == "DOWN")
        probability = up / (up + down)
        score = abs(probability - 0.5) * math.sqrt(count)
        candidate = {
            "length": length, "pattern": list(suffix), "count": count, "probability": probability,
            "first_half_probability": p_first, "second_half_probability": p_second, "score": score,
        }
    ...
    return ComponentSignal(
        "pattern", clip(float(best["probability"]), 0.38, 0.62), reliability, True,
        "stable_multilength_pattern", best,
    )
...
def adaptive_weights(
    ...
    if total <= 0:
        return {"price": 1.0, "binance": 0.0, "crowd": 0.0, "m9": 0.0, "pattern": 0.0}
    return {name: value / total for name, value in raw.items()}


def conservative_coefficient(current: Optional[float], total_pool: float, side_pool: float) -> float:
    neutral = settings.neutral_net_coefficient
    if current is None or current <= 1.0:
        return neutral
    pool_confidence = clip(total_pool / max(settings.min_pool_bnb, 0.01), 0.0, 1.0)
    side_confidence = clip(side_pool / max(settings.min_side_pool_bnb, 0.005), 0.0, 1.0)
...
def select_side_and_stake(
    ...
    if ev_up >= ev_down:
        signal, selected_ev, agreement = "UP", ev_up, agreement_up
    else:
        signal, selected_ev, agreement = "DOWN", ev_down, 1.0 - agreement_up
    ...
    if trades >= settings.min_trades_variable_stake:
        if selected_ev >= settings.ev_high_threshold and agreement >= settings.high_agreement:
            ...
            quality = "HIGH_EDGE"
        elif selected_ev >= settings.ev_medium_threshold and agreement >= settings.medium_agreement:
...
def build_decision(
    ...
    raw_probability = sum(weights.get(item.name, 0.0) * item.probability_up for item in components)
    ...
    coeff_up = conservative_coefficient(
        snapshot.current_net_coeff_up, snapshot.betting_total_bnb, snapshot.betting_bull_bnb
    )
    coeff_down = conservative_coefficient(
        snapshot.current_net_coeff_down, snapshot.betting_total_bnb, snapshot.betting_bear_bnb
    )
    choice = select_side_and_stake(probability_up, coeff_up, coeff_down, agreement_up, state)
    return {
        **choice,
